...
def extract_test_results(text):
    ...
    #pattern = r"((?:\b\w+\b\s?){0,1}(?:creatinine|sodium|potassium|chloride|electrolytes|blood urea nitrogen|bun|glomerular filtration rate|gfr))\s+(\d+\.?\d*)"
    pattern = r"([a-z\s/()-]*\b(?:creatinine|sodium|potassium|chloride|electrolytes|blood urea nitrogen|bun|glomerular filtration rate|gfr)\b[a-z\s/()-]*)\s+(\d+\.?\d*)(?:\s+(?:mg/dL|mmol/L|RATIO))?"
